...
class Genlib
{
  ...
  /**
   * redirect to base_url if user role is super/owner/manager/employee
   */
  public function superOnly()
  {
    //prevent access if user is not logged in | if role is "Super" grant full access
    if (empty($_SESSION['admin_id']) || (isset($_SESSION['admin_role']) && $_SESSION['admin_role'] !== "Super")) {
      redirect(base_url());
    }
  }

  public function noEmployee()
  {
    //prevent access if user is an employee | if role is "Super", "manag" or "owner" grant full access
    if (empty($_SESSION['admin_id']) || (isset($_SESSION['admin_role']) && $_SESSION['admin_role'] === "Emplo")) {
      redirect(base_url());
    }
  }

  public function onlyOwnerSuper()
  {
    //prevent access if user is an employee or a manager | if role is "Super" or "owner" grant full access
    if (empty($_SESSION['admin_id']) || (isset($_SESSION['admin_role']) && $_SESSION['admin_role'] === "Emplo") || (isset($_SESSION['admin_role']) && $_SESSION['admin_role'] === "Manag")) {
      redirect(base_url());
    }
  }
